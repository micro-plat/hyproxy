...
func NewApiResponsiveServer(registryAddr string, cnf conf.IServerConf, logger *logger.Logger) (h *ApiResponsiveServer, err error) {
	...
	if h.server, err = NewProxyServer(cnf.GetServerName(),
		cnf.GetString("address", ":8080"),
		nil,
		WithShowTrace(cnf.GetBool("trace", false)),
		WithLogger(logger),
		WithName(cnf.GetPlatName(), cnf.GetSysName(), cnf.GetClusterName(), cnf.GetServerType())); err != nil {
		return
	}
...
func (w *ApiResponsiveServer) Restart(cnf conf.IServerConf) (err error) {
	...
	if w.server, err = NewProxyServer(cnf.GetServerName(),
		cnf.GetString("address", ":8080"),
		nil,
		WithShowTrace(cnf.GetBool("trace", false)),
		WithLogger(w.Logger),
